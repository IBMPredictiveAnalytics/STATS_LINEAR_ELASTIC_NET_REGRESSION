    ...
    def execute_model(data):
        ...
        try:
            ...
            if mode == "FIT":
                ...
                create_cv_output(y, x_names, x_fnotes, y_name, nfolds, intl, output_json, result)
...
def create_cv_output(y, x_names, x_fnotes, y_name, nfolds, intl, output_json, result):
    ...
    if get_value("plot_mse"):
        ...
        plotdata = np.column_stack((result["ratio_out"], result["alpha_out"], result["mean_mse"]))
        ...
        average_mse_chart = GplChart(intl.loadstring("average_mse_line_chart_title").format(result["best_ratio"]))
        graph_dataset = "graphdataset"
        ...
        gpl = ["SOURCE: s=userSource(id(\"{0}\"))".format(graph_dataset),
               ...
               "GUIDE: axis(dim(1), label(\"{0}\"))".format(intl.loadstring("alpha")),
               "GUIDE: axis(dim(2), label(\"{0}\"))".format(intl.loadstring("mse")),
               "GUIDE: text.title(label(\"{0}\"))".format(intl.loadstring("average_mse_line_chart_title").format(result["best_ratio"])),
               "SCALE: {0}".format(scale),
               "SCALE: linear(dim(2), include(0,1))",
               "ELEMENT: line(position(x*y))",
               "ELEMENT: point(position(x*y))"]
        ...
        average_mse_chart.add_variable_mapping("x", list(plotdata[:,1]), graph_dataset)
        average_mse_chart.add_variable_mapping("y", list(plotdata[:,2]), graph_dataset)
        output_json.add_chart(average_mse_chart)

    if get_value("plot_r2"):
        # Create data for plotting varying alpha for best value of ratio.
        plotdata = np.column_stack((result["ratio_out"], result["alpha_out"], result["mean_r2"]))
        # Extract only the lines that have a ratio that matches the best ratio value
        plotdata = plotdata[np.where(plotdata[:, 0] == result["best_ratio"])]

        average_r2_chart = GplChart(intl.loadstring("average_r2_line_chart_title").format(result["best_ratio"]))
        graph_dataset = "graphdataset"
        ...
        gpl = ["SOURCE: s=userSource(id(\"{0}\"))".format(graph_dataset),
               ...
               "GUIDE: text.title(label(\"{0}\"))".format(intl.loadstring("average_r2_line_chart_title").format(result["best_ratio"])),
               "SCALE: {0}".format(scale),
               "SCALE: linear(dim(2), include(0,1))",
